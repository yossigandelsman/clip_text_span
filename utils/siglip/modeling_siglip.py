# Based on https://github.com/huggingface/transformers/blob/v4.52.2/src/transformers/models/siglip/modeling_siglip.py
...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, interpolate_pos_encoding=False) -> torch.Tensor:
        ...
        patch_embeds = self.hook("patch_embedding", ret=self.patch_embedding(pixel_values.to(dtype=target_dtype)))
        embeddings = self.hook("flatten_transpose", ret=patch_embeds.flatten(2).transpose(1, 2))
        if interpolate_pos_encoding:
            pos_emb = self.hook("interpolated_position_embedding", ret=self.interpolate_pos_encoding(embeddings, height, width))
            embeddings = self.hook("add_pos_embedding", ret=embeddings + pos_emb)
        else:
            pos_emb = self.hook("position_embedding", ret=self.position_embedding(self.position_ids))
            embeddings = self.hook("add_pos_embedding", ret=embeddings + pos_emb)
        return embeddings
...
class SiglipAttention(nn.Module):
    ...
    def __init__(self, config: Union[SiglipVisionConfig, SiglipTextConfig], hook: Optional[HookManager] = None):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
class SiglipMLP(nn.Module):
    def __init__(self, config, hook: Optional[HookManager] = None):
        super().__init__()
        self.config = config
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.hook("fc1", ret=self.fc1(hidden_states))
        hidden_states = self.hook("activation", ret=self.activation_fn(hidden_states))
        hidden_states = self.hook("fc2", ret=self.fc2(hidden_states))
        return hidden_states


class SiglipEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Union[SiglipVisionConfig, SiglipTextConfig], hook: Optional[HookManager] = None, layer_idx: Optional[int] = None):
        super().__init__()
        self.hook = hook or HookManager()
        self.embed_dim = config.hidden_size
        self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.self_attn = SiglipAttention(config, hook=self.hook.fork("self_attn") if hook else None)
    ...
    def forward(
        ...
        hidden_states = self.hook("layer_norm1", ret=self.layer_norm1(hidden_states))
        hidden_states, attn_weights = self.hook("ret_attn", ret=self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            output_attentions=output_attentions,
        ))
        hidden_states = self.hook("add_residual1", ret=residual + hidden_states)

        residual = hidden_states
        hidden_states = self.hook("layer_norm2", ret=self.layer_norm2(hidden_states))
        hidden_states = self.hook("ret_mlp", ret=self.mlp(hidden_states))
        hidden_states = self.hook("add_residual2", ret=residual + hidden_states)
...
class SiglipEncoder(nn.Module):
    ...
    def __init__(self, config: SiglipConfig, hook: Optional[HookManager] = None):
        super().__init__()
        self.config = config
        self.hook = hook or HookManager()
        self.layers = nn.ModuleList([
            SiglipEncoderLayer(
                config,
                hook=self.hook.fork(f"encoder_layer.{i}") if hook else None,
                layer_idx=i
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.hook("ret_embeddings", ret=self.embeddings(pixel_values, interpolate_pos_encoding=interpolate_pos_encoding))
        encoder_outputs: BaseModelOutput = self.hook("ret_encoder", ret=self.encoder(
            inputs_embeds=hidden_states,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        ))
        last_hidden_state = self.hook("post_layernorm_input", ret=encoder_outputs.last_hidden_state)
        last_hidden_state = self.hook("post_layernorm", ret=self.post_layernorm(last_hidden_state))
        pooler_output = self.hook("ret_pooling_head", ret=self.head(last_hidden_state) if self.use_head else None)
        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state,
...
class SiglipVisionModel(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipVisionConfig, hook: Optional[HookManager] = None):
        super().__init__(config)
        self.hook = hook or HookManager()
        self.vision_model = SiglipVisionTransformer(config, hook=self.hook)
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipConfig, hook: Optional[HookManager] = None):
        ...
        vision_model = SiglipVisionModel._from_config(vision_config, hook=self.hook)
