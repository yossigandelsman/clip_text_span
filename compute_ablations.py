...
def main(args):
    ...
    mlps_ablation_acc = accuracy(torch.from_numpy(mlps_ablation @ classifier).float(), 
                                 torch.from_numpy(labels))[0]*100
    print('+ MLPs ablation:', mlps_ablation_acc)
    mlps_no_layers = mlps.sum(axis=1)
    attns_no_cls = attns.sum(axis=2) 
    with open(os.path.join(args.input_dir, f'{args.dataset}_cls_attn_{args.model}.npy'), 'rb') as f:
    ...
    no_cls_acc = accuracy(torch.from_numpy(no_cls_ablation @ classifier).float(), 
                          torch.from_numpy(labels))[0]*100
    print('+ CLS ablation:', no_cls_acc)
    mlp_and_no_cls_ablation = attns_no_cls.sum(axis=1) + mlps_mean.sum(axis=1)
    mlp_and_no_cls_ablation_acc = accuracy(
    ...
    for layer in range(attns.shape[1]):
        current_model = (np.sum(np.mean(no_heads_attentions[:, :layer], axis=0, keepdims=True), axis=1) + 
